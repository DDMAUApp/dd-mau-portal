    ...
    const unsubscribe = onSnapshot(doc(db, "config", "staff"), (docSnap) => {
      if (docSnap.exists() && docSnap.data().list) {
        ...
        if (needsMigration) {
          ...
          setStaffList(merged);
          setDoc(doc(db, "config", "staff"), { list: merged }).catch(err =>
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-gradient-to-r from-mint-700 to-mint-600 text-white p-4 sticky top-0 z-40 shadow-lg">
        <div className="max-w-lg mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">{"\u{1F35C}"} DD Mau</h1>
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          {(staffLocation === "both" || staffIsAdmin) ? (
            <div className="flex gap-1">
              <button
                onClick={() => setActiveLocation("webster")}
              ...
              <button
                onClick={() => setActiveLocation("maryland")}
          ...
          <div className="pb-24" style={{background: "#111827"}}>
            <div style={{background: "linear-gradient(135deg, #059669, #047857)", padding: "24px 16px 20px", color: "white"}}>
              <p style={{fontSize: "13px", opacity: 0.8, margin: 0}}>
                {language === "es" ? "Buenas tardes" : new Date().getHours() < 12 ? "Good morning" : new Date().getHours() < 17 ? "Good afternoon" : "Good evening"}
              </p>
            ...
            <div style={{padding: "16px"}}>
              <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px"}}>
                {[
                  { tab: "training", icon: "\u{1F4DA}", label: t("trainingHub", language), sub: language === "es" ? "M\u00F3dulos" : "Modules" },
                  { tab: "operations", icon: "\u{1F4CB}", label: t("dailyOps", language), sub: language === "es" ? "Listas" : "Checklists" },
                  { tab: "menu", icon: "\u{1F35C}", label: t("menuReference", language), sub: language === "es" ? "Men\u00FA completo" : "Full menu" },
                  { tab: "schedule", icon: "\u{1F4C5}", label: t("weeklySchedule", language), sub: language === "es" ? "Tus turnos" : "Your shifts" },
                ].map(b => (
                  <button
                    key={b.tab}
                ...
                {(isAtDDMau || staffIsAdmin) ? (
                  <button
                    ...
                    style={{background: "#1f2937", borderRadius: "16px", padding: "16px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", border: "1px solid #374151", cursor: "pointer"}}
                  >
                    <div style={{width: "44px", height: "44px", background: "#065f46", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px", fontSize: "22px"}}>{"\u{1F9D1}\u{200D}\u{1F373}"}</div>
                  ...
                  <div style={{background: "#1f2937", borderRadius: "16px", padding: "16px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", border: "1px solid #374151", opacity: 0.4}}>
                    <div style={{width: "44px", height: "44px", background: "#374151", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px", fontSize: "22px"}}>{"\u{1F512}"}</div>
                ...
                <button
                  onClick={() => setActiveTab("eighty6")}
                  style={{background: "#1f2937", borderRadius: "16px", padding: "16px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", border: "1px solid #374151", cursor: "pointer"}}
                >
                  <div style={{width: "44px", height: "44px", background: "#991b1b", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px", fontSize: "22px"}}>{"\u{1F6AB}"}</div>
                ...
                {[
                  { tab: "catering", icon: "\u{1F37D}\u{FE0F}", label: language === "es" ? "Catering" : "Catering", sub: language === "es" ? "Pedidos" : "Orders" },
                  { tab: "maintenance", icon: "\u{1F527}", label: language === "es" ? "Mantenimiento" : "Maintenance", sub: language === "es" ? "Reportar" : "Report issue" },
                ].map(b => (
                  <button
                    key={b.tab}
                    onClick={() => setActiveTab(b.tab)}
                ...
                <button
                  ...
                  style={{background: "#1f2937", borderRadius: "16px", padding: "16px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", border: "1px solid #374151", cursor: "pointer"}}
                >
                  <div style={{width: "44px", height: "44px", background: "#065f46", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px", fontSize: "22px"}}>{"\u{1F3E5}"}</div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-gray-200 navbar-shadow">
        <div className="max-w-lg mx-auto flex justify-around items-center h-20">
          {[
            { tab: "home", icon: "\u{1F3E0}", label: t("home", language) },
            { tab: "training", icon: "\u{1F4DA}", label: t("training", language) },
            { tab: "operations", icon: "\u{1F4CB}", label: t("operations", language) },
            { tab: "menu", icon: "\u{1F35C}", label: t("menu", language) },
            { tab: "schedule", icon: "\u{1F4C5}", label: t("schedule", language) },
          ].map(b => (
            <button
              key={b.tab}
              onClick={() => setActiveTab(b.tab)}
          ...
          {(isAtDDMau || staffIsAdmin) && (
            ...
            >
              <span className="text-2xl">{"\u{1F9D1}\u{200D}\u{1F373}"}</span>
