...
function AdminPanelInner({ language, staffName, staffList, setStaffList, storeLocation, onNavigate, hasOnboardingAccess, onSelfRenamed }) {
            ...
            useEffect(() => {
                ...
                // 2026-08-25 perf: these two badges used to hold always-on
                // onSnapshot listeners over the 500 newest onboarding
                // applications + hires — ~1,000 PII-heavy full documents
                // (drawn-signature data URLs included) streamed to every
                // admin visit, re-emitted on every edit, just to compute two
                // numbers. Now: server-side aggregate counts (zero document
                // transfer), refreshed on mount + every 5 min while the
                // panel is open. Count shape: open = total − closed, which
                // correctly treats legacy docs with NO status field as open
                // (a `not-in` filter would silently exclude them).
                // 2026-06-20 (QA audit L3) semantics preserved — on a load
                // error keep the last known count (the badge is a launcher
                // hint; the detail screen re-queries) and just log.
                let cancelled = false;
                const fetchCounts = async () => {
                    try {
                        const appsCol = collection(db, 'onboarding_applications');
                        const closed = ['hired', 'not_selected', 'withdrew', 'expired'];
                        const [allApps, closedApps] = await Promise.all([
                            getCountFromServer(appsCol),
                            getCountFromServer(query(appsCol, where('status', 'in', closed))),
                        ]);
                        if (!cancelled) setOnboardingPendingApps(
                            Math.max(0, allApps.data().count - closedApps.data().count));
                    } catch (err) { console.warn('onboarding apps count load error:', err?.code || err); }
                    try {
                        const hiresCol = collection(db, 'onboarding_hires');
                        const [allHires, doneHires] = await Promise.all([
                            getCountFromServer(hiresCol),
                            getCountFromServer(query(hiresCol, where('status', 'in', ['archived', 'complete']))),
                        ]);
                        if (!cancelled) setOnboardingActiveHires(
                            Math.max(0, allHires.data().count - doneHires.data().count));
                    } catch (err) { console.warn('onboarding hires count load error:', err?.code || err); }
                };
                fetchCounts();
                const id = setInterval(fetchCounts, 5 * 60_000);
            ...
            const resetEditForm = () => {
                ...
                setEditHiddenPages([]);
                // 2026-08-25 — was the ONE field this centralized reset
                // missed (commitStaffEdit writes healthAccess
                // unconditionally, so a stale value here would be a
                // cross-person clobber if any caller skipped re-seeding).
                setEditHealthAccess(false);
            };
        ...
        // (2026-08-25 — ~230 lines of dead CATERING_MENU/ALL_SAUCES/
        // BASE_OPTIONS constants removed. The live copy is
        // src/data/catering.js, imported by CateringOrder.jsx; the
        // duplicate here was a stale-price trap and dead admin-chunk
        // weight.)
