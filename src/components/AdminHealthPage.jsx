...
// (owner-only "🐛 Error Report" tab). This page now stays focused on
// systems status: Firestore liveness, TVs, printers, backups, recent
// audit. The split lets Andrew say "go to the error report tab" without
// ambiguity. See ErrorReportPage.jsx for the full triage UI.
...
export default function AdminHealthPage({ language = 'en', staffName }) {
    ...
    // tab now — see src/components/ErrorReportPage.jsx. This page is
    // systems-status only (Firestore, TVs, printers, backups, audit).
    ...
    return (
        <section className="w-full max-w-6xl mx-auto px-3 md:px-6 py-4 md:py-6 space-y-5">
            ...
            {/* Pointer card — Errors + bug reports + AI failures live
                on the dedicated 🐛 Error Report tab. Andrew 2026-05-27
                split them out so Health stays focused on "is the
                platform up?" and ErrorReport is the triage destination. */}
            <div className="bg-white border border-dd-line rounded-2xl p-4 flex items-center gap-3">
                <div className="text-3xl shrink-0">🐛</div>
                <div className="flex-1 min-w-0">
                    <div className="text-sm font-black text-dd-text">
                        {tx('Errors + bug reports moved', 'Errores + reportes movidos')}
                    </div>
                    <div className="text-[12px] text-dd-text-2 mt-0.5">
                        {tx(
                            'Open the Error Report tab in the sidebar to see open bug reports, recent crashes grouped by signature, AI failures, and a deep link to Sentry.',
                            'Abre la pestaña Errores en el sidebar para ver reportes abiertos, errores recientes, fallos de IA y un enlace a Sentry.',
                        )}
                    </div>
                </div>
            </div>
