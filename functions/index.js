...
exports.dispatchNotification = onDocumentCreated(
    ...
    //
    // 2026-06-03 — APNs secrets attached for direct iOS push delivery
    // via node-apn. CRITICAL DEPLOY GOTCHA: when attaching brand-new
    // secrets to an existing function, Firebase has to grant the SA's
    // secretAccessor role first. Doing 3+ new bindings in ONE deploy
    // hangs the discovery process indefinitely (manifested as the
    // "Loading and analyzing source code" stage timing out after 540s).
    // Workaround: add new secrets ONE AT A TIME, deploying between
    // each. The SA grant completes quickly per-deploy.
    {
